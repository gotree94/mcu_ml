...
[3/6] Float32 TFLite 변환 중...
  ✅ Float32 TFLite 저장 완료: mnist_float32.tflite
     크기: 439,880 bytes (429.6 KB)

[4/6] Int8 양자화 TFLite 변환 중...
  ✅ Int8 양자화 TFLite 저장 완료: mnist_int8_quantized.tflite
     크기: 117,624 bytes (114.9 KB)

[5/6] 모델 크기 비교
  Float32 모델:  439,880 bytes (429.6 KB)
  Int8 양자화 모델:  117,624 bytes (114.9 KB)
  📉 크기 감소율: 73.3%
...
[3/7] Float32 모델 추론 중...
    Float32: 2000/10000 완료 (현재 정확도: 0.9610)
    Float32: 4000/10000 완료 (현재 정확도: 0.9603)
    Float32: 6000/10000 완료 (현재 정확도: 0.9647)
    Float32: 8000/10000 완료 (현재 정확도: 0.9691)
    Float32: 10000/10000 완료 (현재 정확도: 0.9716)
  ✅ Float32 TFLite 정확도: 0.9716 (97.16%)
     평균 추론 시간: 0.0111 ms

[4/7] Int8 양자화 모델 추론 중...
    Int8: 2000/10000 완료 (현재 정확도: 0.9590)
    Int8: 4000/10000 완료 (현재 정확도: 0.9600)
    Int8: 6000/10000 완료 (현재 정확도: 0.9645)
    Int8: 8000/10000 완료 (현재 정확도: 0.9689)
    Int8: 10000/10000 완료 (현재 정확도: 0.9715)
  ✅ Int8 양자화 TFLite 정확도: 0.9715 (97.15%)
     평균 추론 시간: 0.0060 ms
...
[3] TFLite 변환
    Float32 TFLite: 439,880 bytes (429.6 KB)
    Int8 양자화 TFLite: 117,624 bytes (114.9 KB)
    📉 압축률: 73.3%

[4] TFLite 모델 평가
    Float32 TFLite: 정확도=0.9716, 추론시간=0.011ms
    Int8 TFLite: 정확도=0.9715, 추론시간=0.006ms
...
TFLite Float32            429.6       97.16       0.011 ms
TFLite Int8 (MCU용)       114.9       97.15       0.006 ms
=====================================================================

💡 인사이트
   • Int8 모델은 Float32 대비 크기 73.3% 감소
   • 추론 속도 1.85배 향상
   • 정확도 손실: 0.01%p (97.16% → 97.15%)
   • MCU(ESP32)에 탑재 시 114.9KB면 충분 (ESP32 SRAM: ~320KB)
```
...
|-----------|-------------------|----------------|-------------------|
| **모델 크기** | 437 KB | 430 KB | **115 KB (73%↓)** |
| **파라미터 수** | 109,386 | 109,386 | 109,386 |
| **데이터 타입** | float32 | float32 | **int8** |
| **테스트 정확도** | 97.16% | 97.16% | **97.15% (-0.01%p)** |
| **추론 시간 (PC)** | ~0.5ms (배치) | 0.011ms | **0.006ms (1.85배↑)** |
| **메모리 요구량** | 437KB+ | 430KB+ | **~115KB** |
...
2. 추론 속도 (Latency)
   Float32: 0.011ms → Int8: 0.006ms
   정수 연산이 실수보다 빠르다.

3. 정확도 (Accuracy)
   97.16% → 97.15% (손실 단 0.01%p)
